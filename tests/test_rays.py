...
def test_computing_a_point_from_a_distance():
    ...
    assert((point(2, 3, 4) == rays.position_fast(origin, direction, 0)).all())
    assert((point(3, 3, 4) == rays.position_fast(origin, direction, 1)).all())
    assert((point(1, 3, 4) == rays.position_fast(origin, direction, -1)).all())
    assert((point(4.5, 3, 4) == rays.position_fast(
